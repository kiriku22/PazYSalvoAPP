...
namespace PazYSalvoAPP.Business.Services
{
    public class PagoService : IPagoService
    {
        ...
        public async Task<bool> Actualizar(Pago model)
        {
            ...
            try
            {
                _context.Pagos.Update(model); // Actualización de la factura en el contexto
        ...
        public async Task<bool> Eliminar(int id)
        {
            ...
            var pagos = _context.Pagos.FirstOrDefault(f => f.Id == id); // Buscar la factura por su ID

            if (pagos == null) return result; // Si la factura no se encontró, devolver el valor por defecto de result (false)

            try
            {
                _context.Pagos.Remove(pagos); // Eliminar la factura del contexto
                await _context.SaveChangesAsync(); // Guardar los cambios en la base de datos
